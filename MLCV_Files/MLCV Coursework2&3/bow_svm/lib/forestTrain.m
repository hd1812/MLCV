function model = forestTrain(X, Y, opts)
    ...
    bagging= false; %param to control bagging
    k=100; %Number of samples for each bootstrap
    ...
    for i=1:numTrees
        ...
        if bagging
            ...
            treeModels{i} = treeTrain(data_train_bagging(rows,1:2), data_train_bagging(rows,3), opts);
        else
