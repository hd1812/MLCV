...
for classf = classifierID
    ...
    if classf == 1
        ...
        for q= 1:numSplits
            ...
            t= rand(1)*(tmax-tmin)+tmin;    %random threshold
            dec = col < t;
        ...
        %show the best information gain
        maxgain;
    elseif classf == 2
    ...
    elseif classf==5
        ...
        for q= 1:numSplits
            ...
            % this is expensive, lets only recompute every once in a while...
            idxes=randsample(2,2)
            x=X(:,idxes);
            d=X(:,idxes(1))-X(:,idxes(2));
            maxd=max(d);
            mind=min(d);

            t= rand(1)*(maxd - mind)+ mind;
            dec= d < t;
            Igain = evalDecision(Y, dec, u);
...
% left or right. u is unique vector of class labels at this node
    ...
    %Entropy and Information gain available here
    YL= Y(dec);
    YR= Y(~dec);
    H= classEntropy(Y, u);
